      ...
      <Page style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.strong}>{props.issueDate}</Text>
          </View>
          <View style={styles.row}>
            <Text>Due Date</Text>
            <Text style={styles.strong}>{props.dueDate}</Text>
          </View>
        </View>
        ...
        <View style={styles.totalsBox}>
          <View style={styles.totalLine}>
            <Text>Subtotal</Text>
            <Text style={styles.strong}>{eur(subtotal)}</Text>
          </View>
          <View style={styles.totalLine}>
            <Text>Tax</Text>
            <Text style={styles.strong}>{eur(props.tax)}</Text>
          </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            <Text>IBAN</Text>
            <Text style={styles.strong}>{IBAN}</Text>
          </View>
  ...
  try {
    ...
    if (!process.env.EMAIL_FROM) {
      return NextResponse.json(
        {
          error:
            "Missing EMAIL_FROM. Set it in Vercel to something like: Bweeng Celtic FC <billing@yourdomain.com>",
        },
        { status: 500 }
      );
    }
    ...
    if (
      ...
      items.some(
        (x) =>
          ...
          !Number.isFinite(x.unit) ||
          x.qty <= 0 ||
          x.unit < 0
      )
    ) {
      return NextResponse.json(
        { error: "Add at least 1 valid line item" },
        { status: 400 }
      );
    }

    const { data: invoiceNumber, error: numErr } = await supabaseAdmin.rpc(
      "next_doc_number",
      { doc_type: "invoice" }
    );

    if (numErr) {
      throw new Error(numErr.message);
    }
    ...
    if ((sendResult as any)?.error) {
      ...
      return NextResponse.json(
        { error: (sendResult as any).error.message || "Email send failed" },
        { status: 500 }
      );
    }
